...
export function StateCard({
  ...
  return (
    <div className={cn('bg-card border border-border/50 rounded-xl', compact ? 'p-3' : 'p-4')}>
      <div className={cn('flex items-center justify-between', compact ? 'mb-2' : 'mb-3')}>
        <span className="text-xs text-muted-foreground">{formatStateDate(stateDate)}</span>
        <span className="text-xs font-medium text-foreground">Сон: {formatSleepLabel(state.sleepMinutes)}</span>
      </div>
